		...
		if frame.USE_PORTRAIT then
			...
			if frame.PORTRAIT_DETACHED and frame.db.portrait.style == "3D" then
				if not portrait.Holder or (portrait.Holder and not portrait.Holder.mover) then
					...
					E:Size(portrait.Holder, frame.db.portrait.detachedWidth, frame.db.portrait.detachedHeight)

					if frame.ORIENTATION == "LEFT" then
						E:Point(portrait.Holder, "RIGHT", frame, "LEFT", -frame.BORDER, 0)
					elseif frame.ORIENTATION == "RIGHT" then
						E:Point(portrait.Holder, "LEFT", frame, "RIGHT", frame.BORDER, 0)
					end

					E:SetInside(portrait, portrait.Holder)
					E:SetOutside(portrait.backdrop, portrait)
...
hooksecurefunc(UF, "Configure_Power", function(_, frame)
	if frame.unitframeType == "player" then
		...
		if db.energyTickEnable then
			...
			E:Size(power.EnergyTick.Spark, frame.POWERBAR_HEIGHT * 1.6, 22)
			...
			power.EnergyTick:SetScript("OnEvent", function()
			...
			power.EnergyTick:SetScript("OnUpdate", function()
				...
				local tickPosition = (frame.POWERBAR_WIDTH / 200 * floor((GetTime() - this.lastTick) * 100) + 0.5)
				E:Point(this.Spark, "LEFT", power, tickPosition - 12, 0)
			end)
		else
